...
export default function Layout() {
    ...
    const pageTitle = PAGE_TITLES[location.pathname] ??
        Object.entries(PAGE_TITLES).find(([path]) => location.pathname.startsWith(path) && path.length > 1)?.[1] ??
        "WAPE";
    return (_jsxs("div", { className: "min-h-screen bg-background flex", children: [_jsx(Sidebar, { collapsed: collapsed, onToggle: () => setCollapsed(!collapsed) }), _jsxs("div", { className: cn("flex-1 flex flex-col min-h-screen transition-all duration-300", collapsed ? "lg:ml-[68px]" : "lg:ml-[260px]"), children: [_jsx(TopBar, { onToggleSidebar: () => setCollapsed(!collapsed), pageTitle: pageTitle }), _jsx("main", { className: "flex-1 overflow-auto p-4 md:p-6", children: _jsx(Outlet, {}) })] })] }));
}
