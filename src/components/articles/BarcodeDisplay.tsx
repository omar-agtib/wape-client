...
export default function BarcodeDisplay({
  ...
  useEffect(() => {
    if (!barcodeId || !svgRef.current) return;
  ...
  const downloadPDF = async (perPage: number = 8) => {
  ...
  if (error)
    return (
      <p className="text-xs text-muted-foreground font-mono">{barcodeId}</p>
    );
      ...
      {showDownload && (
        <div className="flex gap-2 flex-wrap justify-center">
          ...
          <Button
            ...
            size="sm"
            className="h-7 text-xs gap-1"
            onClick={() => downloadPDF(8)}
          >
            <Download className="w-3 h-3" /> PDF (8/page)
          </Button>
          <Button
            variant="outline"
            size="sm"
            className="h-7 text-xs gap-1"
            onClick={() => downloadPDF(16)}
          >
            <Download className="w-3 h-3" /> PDF (16/page)
          </Button>
        </div>
      )}
    </div>
